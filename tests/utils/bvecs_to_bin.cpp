...
void block_convert(std::ifstream& reader, std::ofstream& writer,
                   uint8_t* read_buf, uint8_t* write_buf, _u64 npts, _u64 ndims) {
  reader.read((char*) read_buf,
              npts * (ndims * sizeof(uint8_t) + sizeof(unsigned)));
...
int main(int argc, char** argv) {
  if (argc != 4 && argc != 3) {
    std::cout << argv[0] << " input_bvecs output_bin (data_size)" << std::endl;
    exit(-1);
  }
  ...
  _u64 npts = fsize / ((ndims + 1) * sizeof(uint8_t));
  if (argc == 4) npts = std::stoi(argv[3]);
  ...
  uint8_t* read_buf = new uint8_t[npts * (ndims + 1)];
