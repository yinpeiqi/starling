...
namespace diskann {
  ...
  template<typename T>
  void SepDiskIndex<T>::page_search(
            ...
            for (unsigned j = 0; j < p_size; ++j) {
              ...
              if (id != exact_id) {
                if (dist_scratch[j] >= retset[cur_list_size - 1].distance * pq_filter_ratio \
                  ...
                  auto fn = std::make_shared<FrontierNode>(id, pid, disk_fid);
            ...
            while (marker < cur_list_size && num_seen < beam_width) {
              if (retset[marker].flag) {
                if (exact_visited.find(retset[marker].id) == exact_visited.end()) {
                  num_seen++;
                  unsigned pid = id2page_[retset[marker].id];
                  ...
                  auto fn = std::make_shared<FrontierNode>(retset[marker].id, pid, disk_fid);
                  frontier.push_back(fn);
