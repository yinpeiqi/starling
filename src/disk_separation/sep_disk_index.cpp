...
namespace diskann {
  template<typename T>
  SepDiskIndex<T>::SepDiskIndex(std::shared_ptr<FileIOManager> &IOManager,
                                diskann::Metric m)
  ...
  template<typename T>
  void SepDiskIndex<T>::setup_thread_data(_u64 nthreads) {
  ...
  template<typename T>
  void SepDiskIndex<T>::destroy_thread_data() {
  ...
  template<typename T>
  void SepDiskIndex<T>::use_medoids_data_as_centroids() {
    if (centroid_data != nullptr)
      aligned_free(centroid_data);
  ...
  template<typename T>
  void SepDiskIndex<T>::load_mem_index(Metric metric, const size_t query_dim, 
...
#ifdef EXEC_ENV_OLS
  template<typename T>
  int SepDiskIndex<T>::load(MemoryMappedFiles &files, uint32_t num_threads,
                            const char *index_prefix) {
#else
  template<typename T>
  int SepDiskIndex<T>::load(uint32_t num_threads, const char *index_prefix,
...
// read index metadata
  ...
  template<typename T>
  void SepDiskIndex<T>::load_partition_data(const std::string &index_prefix,
  ...
  // instantiations
  template class SepDiskIndex<_u8>;
  template class SepDiskIndex<_s8>;
  template class SepDiskIndex<float>;
