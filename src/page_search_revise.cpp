...
namespace diskann {
  ...
  void PQFlashIndex<T>::page_search(
    ...
    // std::unordered_set<unsigned> last_io_nbrs;
    std::vector<std::vector<_u64>> last_io_nbrs;
    last_io_nbrs.reserve(beam_width);
        ...
        // if (page_visited.find(pid) == page_visited.end() && retset[marker].flag) {
          ...
          if (iter != nhood_cache.end()) {
            ...
            if (fetch_last_io_nbrs) {
                for (int i = 0; i < last_io_pids.size(); i++) {
                    if (pid == last_io_pids[i]) {
                        in_last_io = true;
                        last_io_nhoods.push_back(std::make_pair(retset[marker].id, i));
                        break;
                    }
                }
            }
...
#pragma omp critical
    ...
    //   for (unsigned i = 0; i < n_frontier; i++) {
